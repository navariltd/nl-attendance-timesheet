...
def calculate_holiday_hours(entry):
    if entry.out_time and entry.shift_start_time:
        in_time_dt = datetime.strptime(
            str(entry.in_time).split(".")[0], "%Y-%m-%d %H:%M:%S"
        )
        out_time_dt = datetime.strptime(
            str(entry.out_time).split(".")[0], "%Y-%m-%d %H:%M:%S"
        )
...
def get_from_time_and_hours(entry):
    ...
    if entry.out_time and entry.shift_end_time:
        ...
        check_out_time = datetime.strptime(
            check_out_time_str, "%Y-%m-%d %H:%M:%S"
        ).time()
        shift_end_time = datetime.strptime(str(entry.shift_end_time), "%H:%M:%S").time()
        overtime_threshold = frappe.db.get_single_value(
            SETTINGS_DOCTYPE, "overtime_threshold"
        )
